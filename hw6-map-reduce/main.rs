...
fn main() {
    ...
    let mut threads: Vec<JoinHandle<usize>> = Vec::new(); // Create thread vector that will hold all the threads
    let mut temp; // temporary variable to store the data information
    for i in 0..2{
        temp = xs[i].clone(); // clone already existing data for no pointer errors
        threads.push(thread::spawn(move || map_data(&temp))); // activate the thread and store the address in vector
    }
    for i in 0..2{
        intermediate_sums.push(threads.remove(0).join().unwrap()); // retrieve the return values of the vector from the thread pointer
    }
    ...
    let xs2 = partition_data(num_partitions, &v); // call partition_data that gives us the vector with as many partitions as user desires
    print_partition_info(&xs2); // print out the partition_info 

    let mut intermediate_sums2 : Vec<usize> = Vec::new(); // this is the vector that holds all the partition's sums

    let mut threads2: Vec<JoinHandle<usize>> = Vec::new(); // creating the vector that holds all the thread pointers
    let mut temp2; // stores a copy of the partitioned data
    for i in 0..num_partitions{
        temp2 = xs2[i].clone(); // copy the partitioned data
        threads2.push(thread::spawn(move || map_data(&temp2))); // store thread address
    }
    for i in 0..num_partitions{
        intermediate_sums2.push(threads2.remove(0).join().unwrap()); // retrieves intermediate sums
    }
    println!("Intermediate sums = {:?}", intermediate_sums2); // prints intermediate sums

    let sum = reduce_data(&intermediate_sums2); // sums the threads together and presents to user
    println!("Sum = {}", sum);
}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    let mut xs: Vec<Vec<usize>> = Vec::new(); // the vector that will hold the partitioned
    let mut counter = 0; // iterates over the initially generated data to update values in dummy
    let mut switch = 0; // indicates whether extra data has fully been accounted for
    let size_partition = v.len()/num_partitions; // the partition size, decided based on the user
    
    let mut extra =  v.len() % num_partitions; // after partition size is determined, the extra numbers that don't fit in an array need to be accounted for
    for i in 0..num_partitions{
        let mut dummy : Vec<usize> = Vec::new(); // create a dummy vector that will be filled with values to be then pushed into xs
        for j in 0..size_partition{
            if counter != v.len(){ // if you reached the end of all the partitions
                if extra > 0 && switch == 0{ // take into account the extra numbers that need to be summed
                    dummy.push(v[counter]); // add them to the dummy vector to be added into xs
                    counter += 1; // iterate the master count
                    extra -= 1; // when extra is 0, the extra numbers are completely taken into account
                    switch = 1; // "activate" this portion indicating program reached the last set of numbers
                }
            dummy.push(v[counter]); // append the values into the dummy variable
            counter += 1; // iterate counter
            }
        }
        switch = 0; // deactivate switch
        xs.push(dummy); // add the partition to the final vector that has a vector of partitions
    }
    xs // return
}
